      ...
      >
        ...
        <div class="mt-2 grid gap-2 md:grid-cols-2 text-sm">
          <div>
            ...
            <div class="font-medium">{{ item.pickup_location || item.delivery?.pickup_location || '—' }}</div>
          </div>
          <div>
            <div class="text-gray-500">Delivery</div>
            <div class="font-medium">{{ item.delivery?.delivery_location || '—' }}</div>
          </div>
        </div>

        <div class="mt-2 space-y-2">
          <div class="text-sm font-medium">Packages ({{ item.packages?.length || 0 }})</div>
          <div v-for="pkg in item.packages" :key="pkg.id" class="flex items-center justify-between rounded border p-2 text-sm">
            <div class="flex items-center gap-3">
              <span class="px-2 py-0.5 rounded bg-gray-100">ID: {{ pkg.id ?? '—' }}</span>
              <span class="text-gray-600">Status: {{ pkg.delivery_status ?? '—' }}</span>
              <span v-if="pkg.weight" class="text-gray-500">• {{ pkg.weight }} {{ pkg.weight_unit }}</span>
            </div>
            <div v-if="enableActions" class="flex gap-2">
              <button class="h-8 px-3 rounded border bg-white hover:bg-gray-50 disabled:opacity-50" :disabled="actionLoading" @click="markPickedUp(pkg.id)">Pick up</button>
              <button class="h-8 px-3 rounded bg-green-600 text-white hover:bg-green-700 disabled:opacity-50" :disabled="actionLoading" @click="openDeliver(pkg.id)">Deliver</button>
            </div>
          </div>
        </div>
      </div>
